...
const useCfg      =   (argv.cfg
				    && argv.cfg.match(/^[a-z]+$/)
				    && fs.existsSync(cfgFilePath)) ? true : false;
...
async function postToAnidex() {
	...
	if (postDx.res && statCodeCheck(postDx.res, 200)) {
		if (argv.debug) {
		   ...
		   && postDx.res.body
		   && postDx.res.body.match(/<title>(.*)<\/title>/)) {
...
async function postToNyaa() {
	...
	if ( statCodeCheck(postNt.res, 200)
	  || statCodeCheck(postNt.res, 400)) {
		...
		if (res.id) {
			...
			await postToTT(res.id);
			   ...
			   && res.errors.torrent
			   && res.errors.torrent.join(' / ').match(/This torrent already exists/)) {
		  ...
		  && postNt.res.body
		  && postNt.res.body.match(/<title>(.*)<\/title>/)) {
...
async function postToPantsu() {
	...
	if ( statCodeCheck(postPt.res, 200)
	  || statCodeCheck(postPt.res, 400)) {
		...
		if (res.id) {
			...
			console.log('[NyaaPantsu]', res.url);
			   ...
			   && res.errors.join(' / ').match(/That torrent already exists/)) {
			let errStr = res.errors.torrent.join('\r\n[NyaaPantsu] ');
		   ...
		   && postPt.res.body
		   && postPt.res.body.match(/<title>(.*)<\/title>/)) {
